...
is subject to change, as this feature is in public beta.

:::

In addition, moderation actions (i.e., ban and timeout) in one channel are automatically applied to all channels in the Shared Chat session.
Currently, individual message deletions (`CLEARMSG`) are not forwarded, but this is likely to change in the future.
Lastly, `/warn` is not forwarded, but this is not expected to change in the near term.
...
8. Since this feature is complex and in open beta, there may be bugs in Twitch's implementation.
