...
# Drops Entitlements - Gets a list of entitlements for a given organization, that have been granted to a game, user or both.
...
resultList.getData().forEach(drop -> System.out.printf("User ID: %s - Status: %s", drop.getUserId(), drop.getFulfillmentStatus()));
```
...
resultList.getData().forEach(drop -> System.out.printf("Game ID: %s - Status: %s", drop.getGameId(), drop.getFulfillmentStatus()));
```
...
resultList.getData().forEach(drop -> System.out.printf("User ID: %s - Timestamp: %s", drop.getUserId(), drop.getUpdatedAt()));
```
</TabItem>
...
resultList.data.foreach{drop -> printf("User ID: ${drop.userid} - Status: ${drop.fulfillmentstatus}")}
```
...
resultList.data.forEach { drop -> printf("Game ID: ${drop.gameid} - Status: ${drop.fulfillmentstatus}")}
```
...
resultList.data.forEach {  drop -> printf("User ID: ${drop.userid} - Timestamp:${drop.updateat}")}
```
</TabItem>
...
resultList.data.each {
    drop -> System.out.printf "User ID: ${drop.userid} - Status: ${drop.fulfillmentstatus}" 
    }
```
...
resultList.data.each {
    drop -> System.out.printf "Game ID: ${drop.gameid} - Status: ${drop.fulfillmentstatus}"
}
```
...
resultList.data.each {
    drop -> System.out.printf "User ID: ${drop.userid} - Timestamp:${drop.updateat}"
    }
```
</TabItem>
</Tabs>
